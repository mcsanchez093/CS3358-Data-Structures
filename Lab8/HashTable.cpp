...
{
    size_type n_capacity = next_prime(this->cap()+1);
    HashTable* n_table = new HashTable(capacity);
    for (size_type i = 0; i < capacity; ++i){
        strcpy(n_table->data[i].word, data[i].word);
    }
    *this = HashTable(n_capacity);
    this->capacity = n_capacity;
    for (size_type i = 0; i < n_capacity; ++i){
        strcpy(data[i].word, n_table->data[i].word);
    }
    delete n_table;
}
...
bool HashTable::search(const char* cStr) const
{
    ...
    size_type pos = hash(cStr);
    if(!strcmp(data[pos].word, cStr)) return true;
    return false;
}
...
{
    if(this->search(cStr)) return;
    this->used++;
    size_type position = hash(cStr);
    strcpy(data[position].word, cStr);
    double thresh_hold = 0.45;
    if(this->load_factor() > thresh_hold){
        this->rehash();
    }
}
